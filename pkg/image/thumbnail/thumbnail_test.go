...
func TestImageDecoding(t *testing.T) {
	...
	for _, img := range images {
		s3val, err := aws.GetTestMaterial("images", img)
		if err != nil {
			t.Error(err)
			return
		}

		img, err := New(s3val.Bytes())
		if err != nil {
			t.Error(err)
			return
		}
		...
		fmt.Printf("b of type > %T - success\n", b)
	}
}

func TestNewDecodingFromString(t *testing.T) {
